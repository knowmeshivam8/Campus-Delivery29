...
import { ChevronLeft, CreditCard, Wallet, Truck, Receipt, CheckCircle2, Plus, Minus, Info } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-[#FFD54F] font-sans">
            ...
            <header className="py-4 px-6 flex items-center justify-between sticky top-0 bg-[#FFD54F] z-30">
                <button onClick={() => router.back()} className="bg-white text-black p-3 rounded-2xl shadow-lg">
                    <ChevronLeft size={24} />
                </button>
                <h1 className="font-black text-2xl uppercase text-[#774f14] italic tracking-tighter">CART & PAYMENT</h1>
                <div className="w-12" />
            </header>
            ...
            <main className="px-4 grid grid-cols-1 md:grid-cols-2 space-y-2 gap-2">
                ...
                >
                    <div className="flex items-center gap-2 mb-3 border-b pb-4 border-gray-100">
                        ...
                        <h2 className="font-black text-gray-900 uppercase tracking-tight">Review Items</h2>
                    </div>
                            ...
                            {cartItems.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3 bg-white px-3 py-2 rounded-xl shadow-sm border border-gray-100">
                                        ...
                                        <button onClick={() => updateQty(item.id, 1)} className="text-gray-400 hover:text-green-500 transition-colors">
                ...
                <div>
                    ...
                    <motion.div
                        ...
                        className="bg-white rounded-[2.5rem] p-6 shadow-xl border border-white"
                    >
                        <h2 className="font-black text-gray-900 uppercase tracking-tight mb-6">Select Payment</h2>
                        <div className="space-y-3">
                            {[
                                { id: 'UPI', label: 'UPI (GooglePay / PhonePe)', icon: <Wallet size={20} /> },
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`font-black text-xs uppercase ${paymentMethod === method.id ? 'text-red-700' : 'text-gray-500'}`}>
                                            {method.label}
                                        </span>
                                    </div>
                                    <input
                                    ...
                                    />
                                    {paymentMethod === method.id && <CheckCircle2 className="text-red-500" size={20} />}
                                </label>
                            ))}
                        </div>
                    </motion.div>
                    ...
                    <div className='flex justify-center'>
                        <motion.button
                            ...
                            className={`w-[70%] py-3 mt-4 rounded-[2.5rem] font-black text-lg shadow-2xl transition-all relative overflow-hidden ${isProcessing ? 'bg-green-500' : 'bg-black text-white'
                                }`}
                        >
                            {isProcessing ? (
                                <motion.div
